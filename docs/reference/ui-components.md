...
- **Other data store records table**: records that exist but were not created here, each badged
  *Not managed here*, shown with Owner and Modified. **No Replace button**, and deletion requires
  typing the key
...
An entry that would overwrite a record the tool does not manage gets an additional *not managed here*
