...
describe('command builders emit the exact wire shape', () => {
  ...
  it('sinkRoute omits sourcePortKey, letting the processor discover the path', () => {
    ...
    ).not.toHaveProperty('sourcePortKey');
  });

  it('sinkRoute carries a multiview-qualified port key verbatim', () => {
    expect(
      sinkRouteCommand('nvx-decoder-1', 'tile2:tileInput', 'cam-1', 'Video')
        .inputPortKey
  ...
  it('clearMidpointOutput', () => {
    expect(
      clearMidpointOutputCommand('dm-chassis-1', 'outputCard5', 'AudioVideo')
    ).toEqual({
...
describe('describeRoutingError', () => {
  it("prefers the endpoint's structured message", () => {
    const error = {
      ...
      data: {
        ...
        error: {
          code: 'noRouteFound',
          message: 'No path from laptop-1 to display-1 for Video.',
        },
      },
    };
    expect(describeRoutingError(error)).toBe(
      'No path from laptop-1 to display-1 for Video.'
    );
  });

  it('falls back to the status code when there is no body', () => {
    expect(describeRoutingError({ status: 500 })).toBe(
      'Routing command failed (500).'
    );
  });
  ...
  it('reports a transport failure plainly', () => {
    ...
    expect(describeRoutingError(undefined)).toBe(
      'Could not reach the processor.'
    );
  });
});

describe('supportsRoutingCommand', () => {
  const routes = (...urls: string[]) => urls.map((Url) => ({ Url }));

  it('detects the routing command route', () => {
    expect(
      supportsRoutingCommand(routes('/cws/app01/api/routingCommand'))
    ).toBe(true);
    expect(supportsRoutingCommand(routes('/api/routingCommand/'))).toBe(true);
    expect(supportsRoutingCommand(routes('routingCommand'))).toBe(true);
  });
  ...
  it('is not fooled by an unrelated route containing the substring', () => {
    expect(supportsRoutingCommand(routes('/api/notRoutingCommand'))).toBe(
      false
    );
    expect(supportsRoutingCommand(routes('/api/routingCommands'))).toBe(false);
    expect(supportsRoutingCommand(routes('/api/routingCommandLog'))).toBe(
      false
    );
  });

  it('is false with no route table', () => {
    expect(supportsRoutingCommand(undefined)).toBe(false);
    expect(supportsRoutingCommand([])).toBe(false);
    expect(supportsRoutingCommand([{}])).toBe(false);
  });
});
