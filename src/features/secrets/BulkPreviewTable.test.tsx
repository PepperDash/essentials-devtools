...
import BulkPreviewTable, { BulkPreviewTableProps } from './BulkPreviewTable';
...
function renderTable(overrides: Partial<BulkPreviewTableProps> = {}) {
  ...
  const props: BulkPreviewTableProps = {
    ...
    warnings: [],
    unmanagedIndices: new Set<number>(),
    overwrite: false,
    onOverwriteChange,
    ...overrides,
  };
...
describe('rows and badges', () => {
  ...
  it("shows each entry's note", () => {
    const results = [
      entry(0, 'a', 'skip', {
        message: 'Already exists; enable overwrite to replace it.',
      }),
    ];
    renderTable({ results, summary: summaryFor(results) });
  ...
  it('renders a placeholder for a blank key rather than an empty cell', () => {
    const results = [
      entry(0, '', 'invalid', { message: 'A secret key is required.' }),
    ];
    renderTable({ results, summary: summaryFor(results) });
...
describe('summary', () => {
  ...
  it('says so when a file would write nothing', () => {
    ...
    expect(
      screen.getByText('Nothing in this file would be written.')
    ).toBeInTheDocument();
  });
});
...
describe('overwrite control', () => {
  it('reports a change so the caller can re-run the preview', () => {
    ...
    fireEvent.click(
      screen.getByLabelText('Replace secrets that already exist')
    );
...
describe('unmanaged targets', () => {
  it('flags the entries the processor reported as unmanaged', () => {
    const results = [
      entry(0, 'mcTokens', 'overwrite'),
      entry(1, 'mine', 'overwrite'),
    ];
    renderTable({
      results,
      summary: summaryFor(results),
      unmanagedIndices: new Set([0]),
    });

    expect(
      within(row('mcTokens')).getByText('not managed here')
    ).toBeInTheDocument();
    expect(
      within(row('mine')).queryByText('not managed here')
    ).not.toBeInTheDocument();
  });

  it('flags an unmanaged entry that is being held back, too', () => {
    const results = [
      entry(0, 'mcTokens', 'skip', { reason: 'unmanagedTarget' }),
    ];
    renderTable({
      results,
      summary: summaryFor(results),
  ...
  // Matched by result index, not provider + key: the processor reports each entry under the
  // provider named in the file, which need not be the one the caller looked up.
  it('matches by result index regardless of the provider shown', () => {
    const results = [
      entry(0, 'shared', 'overwrite', { provider: 'CrestronGlobalSecrets' }),
    ];
    renderTable({
      results,
      summary: summaryFor(results),
...
describe('client-side warnings', () => {
  it('lists problems found while parsing the file', () => {
    ...
    expect(
      screen.getByText('"dup" appears more than once.')
    ).toBeInTheDocument();
  });
