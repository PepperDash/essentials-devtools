...
const codes = (result: ReturnType<typeof parse>) =>
  result.ok ? result.warnings.map((w) => w.code) : result.issues.map((i) => i.code);
...
describe("file-level rejection", () => {
  ...
  it("rejects a file with too many entries", () => {
    const many = Array.from({ length: MAX_ENTRIES + 1 }, (_, i) => ({ key: `k${i}`, value: "v" }));
...
describe("malformed JSON", () => {
  it("reports a location without using the parser's own message", () => {
    ...
    expect(result.issues[0].message).toMatch(/not valid JSON \(line \d+, column \d+\)/);
  });
...
describe("accepted root shapes", () => {
  ...
  it("accepts the flat template map and picks up its provider", () => {
    const result = parse(
      json({ provider: "default", secrets: { displayPassword: "hunter2", codecPassword: "s3cret" } }),
    );
...
describe("entry validation", () => {
  ...
  it("rejects control characters in a key", () => {
    expect(codes(parse(json([{ key: "bad\nkey", value: "v" }])))).toEqual(["badKeyChars"]);
  });

  it("rejects the API's own reserved records", () => {
    expect(codes(parse(json([{ key: "__essSecretsIdx", value: "v" }])))).toEqual(["reservedKey"]);
    expect(codes(parse(json([{ key: "__essSecretsIdx03", value: "v" }])))).toEqual(["reservedKey"]);
  });

  it("rejects a non-string value", () => {
    expect(codes(parse(json([{ key: "k" }])))).toEqual(["missingValue"]);
    expect(codes(parse(json([{ key: "k", value: 42 }])))).toEqual(["missingValue"]);
    expect(codes(parse(json([{ key: "k", value: null }])))).toEqual(["missingValue"]);
  });
  ...
  it("rejects a blank value, because blank means DELETE on the processor", () => {
    const result = parse(json([{ key: "keep", value: "v" }, { key: "k", value: "" }]));
  ...
  it("rejects an invalid provider or description", () => {
    expect(codes(parse(json([{ key: "k", value: "v", provider: "" }])))).toEqual(["badProvider"]);
    expect(codes(parse(json([{ key: "k", value: "v", description: 5 }])))).toEqual([
      "badDescription",
    ]);
  });
  ...
  it("trims key and provider but never the value", () => {
    const result = parse(json([{ key: "  k  ", value: "  v  ", provider: "  default  " }]));
    expect(result.ok && result.entries).toEqual([
  ...
  it("ignores unrecognized properties with a warning rather than failing", () => {
    const result = parse(json([{ key: "k", value: "v", note: "hi", extra: 1 }]));
    expect(result.ok).toBe(true);
    if (!result.ok) return;
...
describe("partial success", () => {
  it("keeps the valid entries and reports the failures alongside", () => {
    ...
    expect(result.warnings.map((w) => w.code)).toEqual(["missingKey", "emptyValue"]);
  });

  it("reports the row index so the UI can point at the offending line", () => {
    const result = parse(json([{ key: "ok", value: "v" }, { key: "", value: "v" }]));
  ...
  it("flags an unfilled template as its own distinct mistake", () => {
    const result = parse(json({ provider: "default", secrets: { a: "", b: "" } }));
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.entries).toEqual([]);
  ...
  it("does not flag a partially filled template as unfilled", () => {
    const result = parse(json({ provider: "default", secrets: { a: "filled", b: "" } }));
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.warnings.map((w) => w.code)).not.toContain("templateNotFilled");
  });
});
...
describe("issue messages never contain values", () => {
  it("holds across every failure mode", () => {
    ...
    const files = [
      ...
      json([{ key: "dup", value }, { key: "dup", value }]),
