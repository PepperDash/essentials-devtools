...
const KNOWN_ENTRY_PROPERTIES = new Set(["key", "value", "provider", "description"]);
...
function normalizeRoot(
  ...
  const provider = typeof wrapper.provider === "string" ? wrapper.provider.trim() : undefined;
...
function toRawEntry(raw: unknown, index: number): RawEntry {
  ...
  return {
    ...
    extraProperties: Object.keys(object).filter((name) => !KNOWN_ENTRY_PROPERTIES.has(name)),
  };
}
...
function validateEntry(
...
): { entry: BulkSecretEntry; warnings: SecretsFileIssue[] } | { issue: SecretsFileIssue } {
  const at = { index: raw.index };

  if (raw.key === undefined && raw.value === undefined) {
    return { issue: issue("notAnObject", `Entry ${raw.index + 1} is not an object.`, at) };
  }

  if (typeof raw.key !== "string" || raw.key.trim() === "") {
    return { issue: issue("missingKey", `Entry ${raw.index + 1} has no key.`, at) };
  }
  const key = raw.key.trim();
  const named = { ...at, key };

  if (CONTROL_CHARS.test(key)) {
    return { issue: issue("badKeyChars", `Key "${key}" contains control characters.`, named) };
  }
  if (key.startsWith(RESERVED_KEY_PREFIX)) {
    return {
      issue: issue("reservedKey", `Key "${key}" is reserved for internal use.`, named),
    };
  }

  if (typeof raw.value !== "string") {
    return {
      issue: issue("missingValue", `"${key}" has no value, or its value is not text.`, named),
    };
  }
  if (raw.value === "") {
    return {
  ...
  if (raw.provider !== undefined) {
    if (typeof raw.provider !== "string" || raw.provider.trim() === "") {
      return { issue: issue("badProvider", `"${key}" has an invalid provider.`, named) };
    }
    provider = raw.provider.trim();
  }

  if (raw.description !== undefined && typeof raw.description !== "string") {
    return { issue: issue("badDescription", `"${key}" has an invalid description.`, named) };
  }
...
export function parseSecretsFile(input: ParseInput): ParseSecretsFileResult {
  ...
  try {
    ...
    return { ok: false, issues: [issue("invalidJson", describeJsonSyntaxError(error, text))] };
  }
...
export function summarizeSecretsFile(entries: BulkSecretEntry[]): SecretsFileSummary {
