...
const keysOf = (candidates: { deviceKey: string }[]) =>
  candidates.map((c) => c.deviceKey).sort();
...
describe('findCandidateSources', () => {
  it('finds a directly tied source', () => {
    ...
    expect(
      keysOf(findCandidateSources(index, 'display', 'hdmi1', 'AudioVideo'))
    ).toEqual(['laptop']);
  });
  ...
  it('gates on signal type, with AudioVideo satisfying both halves', () => {
    ...
    expect(
      keysOf(findCandidateSources(index, 'display', 'hdmi1', 'Video'))
    ).toEqual(['av-src', 'cam']);
    expect(
      keysOf(findCandidateSources(index, 'display', 'hdmi1', 'Audio'))
    ).toEqual(['av-src', 'mic']);
    // An AudioVideo request matches anything carrying either half.
    expect(
      keysOf(findCandidateSources(index, 'display', 'hdmi1', 'AudioVideo'))
    ).toEqual(['av-src', 'cam', 'mic']);
  });
  ...
  it('constrains on the destination port', () => {
    ...
    expect(
      keysOf(findCandidateSources(index, 'display', 'hdmi2', 'AudioVideo'))
    ).toEqual(['bluray']);
  });
  ...
  it('sorts pure sources first, then by name', () => {
    ...
    expect(
      findCandidateSources(index, 'display', 'hdmi1', 'AudioVideo').map(
        (c) => c.deviceKey
      )
  ...
  it('ignores tie lines naming a device the read API filtered out', () => {
    ...
    expect(
      findCandidateSources(index, 'display', 'hdmi1', 'AudioVideo')
    ).toEqual([]);
  });
...
describe('multiview tiles', () => {
  ...
  const index = buildRouteIndex(
    system(
      [
        ...
        device('nvx-decoder', 'multiview', {
          in: ['tile1:tileInput', 'tile2:tileInput'],
        }),
      ],
      [
  ...
  it("returns every tile's sources when no port is specified", () => {
    expect(
      keysOf(findCandidateSources(index, 'nvx-decoder', null, 'AudioVideo'))
    ).toEqual(['cam-1', 'cam-2']);
  });
});
...
describe('performance', () => {
  it('sweeps a 400-device / ~2000-tie-line system quickly', () => {
    ...
    const candidates = findCandidateSources(
      index,
      'sink-0',
      'hdmi1',
      'AudioVideo'
...
describe('findReachableUpstreamDevices', () => {
  it('returns a set containing only upstream devices for the requested atom', () => {
    ...
    expect(
      Array.from(
        findReachableUpstreamDevices(index, 'display', 'hdmi1', 'Video')
      )
    ).toEqual(['cam']);
    expect(
      Array.from(
        findReachableUpstreamDevices(index, 'display', 'hdmi1', 'Audio')
      )
    ).toEqual(['mic']);
    expect(
      findReachableUpstreamDevices(index, 'display', 'hdmi1', 'Usb').size
    ).toBe(0);
  });
});
...
describe('describeNoCandidates', () => {
  ...
  it('reports an unwired port as a configuration gap', () => {
    expect(
      describeNoCandidates(index, 'display', 'hdmi2', 'AudioVideo')
    ).toMatch(/Nothing is wired to this input/);
  });
  ...
  it('does not claim a narrower overlap is empty, because it is not', () => {
    ...
    expect(
      findCandidateSources(index, 'display', 'hdmi1', 'Video')
    ).toHaveLength(1);
  });
  ...
  it('only ever explains a genuinely empty list', () => {
    expect(
      findCandidateSources(index, 'display', 'hdmi1', 'Audio')
    ).toHaveLength(0);
    expect(
      findCandidateSources(index, 'display', 'hdmi2', 'Video')
    ).toHaveLength(0);
  });
});
