...
import { MidpointRoute, RoutingDevice, RoutingDevicesAndTieLines, TieLine } from "../../store/apiSlice";
...
describe("resolveCurrentSource", () => {
  it("follows the midpoint's active route to the real origin", () => {
    expect(
      resolveCurrentSource(SIMPLE, { mtx: [route("in1", "out1")] }, "display", "hdmi1"),
  ...
  it("follows a midpoint switch without any sink-side feedback", () => {
    expect(
      resolveCurrentSource(SIMPLE, { mtx: [route("in2", "out1")] }, "display", "hdmi1"),
  ...
  it("reports cleared when the feeding output has no active route", () => {
    ...
    expect(
      resolveCurrentSource(SIMPLE, { mtx: [route("in1", "out9")] }, "display", "hdmi1"),
    ).toEqual({ status: "cleared" });
    expect(resolveCurrentSource(SIMPLE, { mtx: [] }, "display", "hdmi1")).toEqual({
      status: "cleared",
    });
  });
  ...
  it("reports unknown when the midpoint publishes no feedback at all", () => {
    ...
    expect(resolveCurrentSource(SIMPLE, {}, "display", "hdmi1")).toEqual({ status: "unknown" });
  });

  it("reports unknown for a port with no tie line to follow", () => {
    expect(resolveCurrentSource(SIMPLE, { mtx: [route("in1", "out1")] }, "display", "hdmi9")).toEqual(
      { status: "unknown" },
    );
  });
  ...
  it("stops at a non-midpoint passthrough, which is the origin as far as routing is concerned", () => {
    const index = buildRouteIndex(
      system(
        [device("cam", "source"), device("scaler", "passthrough"), device("display", "sink")],
        [tie("cam", "out1", "scaler", "in1"), tie("scaler", "out1", "display", "hdmi1")],
      ),
    );
  ...
  it("terminates on a loop instead of hanging", () => {
    const index = buildRouteIndex(
      system(
        [device("mtx-a", "midpoint"), device("mtx-b", "midpoint"), device("display", "sink")],
        [
    ...
    expect(
      resolveCurrentSource(
        index,
        { "mtx-a": [route("in1", "out1")], "mtx-b": [route("in1", "out1"), route("in1", "out2")] },
        "display",
        "hdmi1",
      ),
  ...
  it("resolves a multiview tile by its qualified port key", () => {
    const index = buildRouteIndex(
      system(
        [device("cam-1", "source"), device("mtx", "midpoint"), device("nvx", "sink")],
        [tie("cam-1", "out1", "mtx", "in1"), tie("mtx", "out5", "nvx", "tile2:tileInput")],
      ),
    );
    expect(
      resolveCurrentSource(index, { mtx: [route("in1", "out5")] }, "nvx", "tile2:tileInput"),
