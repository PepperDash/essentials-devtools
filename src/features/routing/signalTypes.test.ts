...
describe("parseSignalFlags", () => {
  it("expands the AudioVideo composite", () => {
    expect(Array.from(parseSignalFlags("AudioVideo"))).toEqual(["Audio", "Video"]);
  });
  ...
  it("tolerates whitespace variations", () => {
    ...
    expect(Array.from(parseSignalFlags("  Audio ,  Video "))).toEqual(["Audio", "Video"]);
  });
...
describe("formatSignalFlags", () => {
  it("collapses {Audio, Video} back to the AudioVideo composite", () => {
    expect(formatSignalFlags(parseSignalFlags("AudioVideo"))).toBe("AudioVideo");
  });
...
describe("flagsContainAll", () => {
  ...
  it("lets an AudioVideo port satisfy a Video request", () => {
    expect(flagsContainAll(parseSignalFlags("AudioVideo"), parseSignalFlags("Video"))).toBe(
      true,
    );
  });

  it("does not let a Video port satisfy an AudioVideo request", () => {
    expect(flagsContainAll(parseSignalFlags("Video"), parseSignalFlags("AudioVideo"))).toBe(
      false,
    );
  });
  ...
  it("is reflexive for every real-world value", () => {
    for (const s of REAL_WORLD) {
      expect(flagsContainAll(parseSignalFlags(s), parseSignalFlags(s))).toBe(true);
    }
  });

  it("keeps Usb isolated from audio and video", () => {
    expect(flagsContainAll(parseSignalFlags("AudioVideo"), parseSignalFlags("Usb"))).toBe(false);
    expect(flagsContainAll(parseSignalFlags("Usb"), parseSignalFlags("Audio"))).toBe(false);
  });

  it("treats an empty request as vacuously satisfied, matching (have & 0) == 0", () => {
    expect(flagsContainAll(parseSignalFlags("Video"), parseSignalFlags(""))).toBe(true);
  });
});
...
describe("flagsIntersect", () => {
  ...
  it("is false for disjoint sets", () => {
    expect(flagsIntersect(parseSignalFlags("Video"), parseSignalFlags("Audio"))).toBe(false);
    expect(flagsIntersect(parseSignalFlags("Video"), parseSignalFlags(""))).toBe(false);
  });
});

describe("signalTypeOptionsForPort", () => {
  it("offers the full type plus each breakaway atom", () => {
    expect(signalTypeOptionsForPort("AudioVideo")).toEqual(["AudioVideo", "Audio", "Video"]);
  });
