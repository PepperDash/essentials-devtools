...
import RoutePopover, { RouteEditTarget, RoutePopoverProps } from "./RoutePopover";
...
const candidate = (deviceKey: string, matchedFlags = ["Audio", "Video"]): CandidateSource => ({
  deviceKey,
  name: deviceKey,
  isPureSource: true,
  matchedFlags,
...
describe("signal-type step", () => {
  ...
  it("collapses to a static label for a single-type port", () => {
    renderPopover({ target: { ...sinkTarget, port: port("hdmiIn1", "Video") } });
  ...
  it("re-queries candidates when the signal type changes", () => {
    const getCandidateSources = vi.fn((signalType: string) =>
      signalType === "Audio" ? [candidate("dsp-1", ["Audio"])] : [candidate("laptop-1")],
    );
...
describe("sink routing", () => {
  ...
  it("submits the narrowed signal type after a breakaway selection", () => {
    ...
    expect(onSubmit).toHaveBeenCalledWith(
      expect.objectContaining({ signalType: "Video", sourceDeviceKey: "laptop-1" }),
    );
  });
  ...
  it("badges a candidate that only has a path for one half of AudioVideo", () => {
    renderPopover({
      getCandidateSources: () => [candidate("cam-1", ["Video"]), candidate("av-1")],
    });
    expect(within(optionButton(/cam-1/)).getByText("video only")).toBeInTheDocument();
...
describe("midpoint routing", () => {
  ...
  it("hides inputs that cannot carry the requested signal type", () => {
    ...
    expect(optionButton(/inputCard1/)).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /inputCard2/ })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /inputCard3/ })).not.toBeInTheDocument();
  });
...
describe("panel styling", () => {
  ...
  it("does not borrow Bootstrap classes that would override its own positioning", () => {
    ...
    for (const cls of ["card", "card-body", "card-header", "position-relative", "position-absolute"]) {
...
describe("submission state and dismissal", () => {
  ...
  it("shows an error without closing", () => {
    renderPopover({ errorMessage: "No path from laptop-1 to display-1 for Video." });
    expect(screen.getByRole("alert")).toHaveTextContent("No path from laptop-1");
...
describe("filtering", () => {
  ...
  it("narrows a long list", () => {
    ...
    fireEvent.change(screen.getByLabelText("Filter options"), { target: { value: "src-19" } });

    expect(optionButton(/src-19/)).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /src-18/ })).not.toBeInTheDocument();
  });

  it("keeps the clear option available while filtering", () => {
    renderPopover({ getCandidateSources: () => many });
    fireEvent.change(screen.getByLabelText("Filter options"), { target: { value: "nothingmatches" } });
