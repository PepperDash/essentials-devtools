...
describe("pendingFromCommand", () => {
  it("watches the destination input and the requested source for a sinkRoute", () => {
    expect(
      pendingFromCommand(sinkRouteCommand("display-1", "hdmiIn1", "laptop-1", "AudioVideo"), NOW),
    ).toEqual({
      deviceKey: "display-1",
      portKey: "hdmiIn1",
      kind: "sinkInput",
  ...
  it("expects the absence of a route for both clear commands", () => {
    const sinkClear = pendingFromCommand(clearSinkCommand("display-1", "hdmiIn1"), NOW);
...
describe("isExpectationMet - sink routes", () => {
  ...
  it("is unmet while nothing is reported", () => {
    expect(isExpectationMet(sinks({}), midpoints({}), routePending)).toBe(false);
  });

  it("is met once the requested source is reported on that port", () => {
    const state = sinks({
      "display-1": [
        { inputPortKey: "hdmiIn1", sourceDeviceKey: "laptop-1", signalType: "AudioVideo" },
      ],
    });
  ...
  it("stays unmet when a different source lands on that port", () => {
    const state = sinks({
      "display-1": [
        { inputPortKey: "hdmiIn1", sourceDeviceKey: "bluray-1", signalType: "AudioVideo" },
      ],
    });
  ...
  it("stays unmet when the right source lands on a different port", () => {
    const state = sinks({
      "display-1": [
        { inputPortKey: "hdmiIn2", sourceDeviceKey: "laptop-1", signalType: "AudioVideo" },
      ],
    });
    expect(isExpectationMet(state, midpoints({}), routePending)).toBe(false);
  });
  ...
  it("resolves a clear only once the entry is gone", () => {
    const clearPending = pendingFromCommand(clearSinkCommand("display-1", "hdmiIn1"), NOW)!;
    ...
    expect(isExpectationMet(stillRouted, midpoints({}), clearPending)).toBe(false);
    ...
    expect(
      isExpectationMet(
        sinks({
          "display-1": [
            { inputPortKey: "hdmiIn2", sourceDeviceKey: "bluray-1", signalType: "AudioVideo" },
          ],
        }),
        midpoints({}),
        clearPending,
  ...
  it("tracks a multiview tile by its qualified port key", () => {
    ...
    const state = sinks({
      "nvx-1": [
        { inputPortKey: "tile1:tileInput", sourceDeviceKey: "cam-1", signalType: "Video" },
        { inputPortKey: "tile2:tileInput", sourceDeviceKey: "cam-2", signalType: "Video" },
      ],
    });
...
describe("isExpectationMet - midpoint routes", () => {
  ...
  it("stays unmet when another output was switched instead", () => {
    const state = midpoints({
      "dm-1": [
        { inputPortKey: "inputCard3", outputPortKey: "outputCard6", signalType: "Video" },
      ],
    });
  ...
  it("resolves a midpoint clear only once that output has no route", () => {
    ...
    expect(isExpectationMet(sinks({}), otherOutputOnly, clearPending)).toBe(true);
  });
});
...
describe("agePendingRoutes", () => {
  ...
  it("flags an entry past the timeout", () => {
    const result = agePendingRoutes({ a: entry(NOW) }, NOW + PENDING_TIMEOUT_MS + 1);
    expect(result.a.timedOut).toBe(true);
  });

  it("does not re-flag an already-flagged entry", () => {
    const pending = { a: entry(NOW, true) };
    expect(agePendingRoutes(pending, NOW + PENDING_TIMEOUT_MS + 1)).toBe(pending);
  });
