...
function system(): RoutingDevicesAndTieLines {
  const port = { signalType: "AudioVideo", connectionType: "Hdmi", isInternal: false };
  return {
    devices: [
      {
        key: "laptop",
...
describe("useRouteCandidates", () => {
  it("returns candidates for a destination port", () => {
    const { result } = renderHook(() => useRouteCandidates(buildRouteIndex(system())));
    expect(result.current("display", "hdmi1", "AudioVideo").map((c) => c.deviceKey)).toEqual([
      "laptop",
    ]);
  });
