...
import { downloadJson, downloadText, timestampedFilename } from "./downloadFile";

// jsdom implements neither of these, so they have to be installed before the helper runs.
// Typed with its parameter so `mock.calls[0][0]` is the Blob rather than an empty tuple.
